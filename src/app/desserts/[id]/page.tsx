...
export default function DishDetail({ params }: { params: { id: string } }) {
  ...
  // Find the dish by ID
  const dish = desserts.find(d => d.id === params.id)
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container py-12 flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-6">The dessert you're looking for doesn't exist or has been removed.</p>
            <Button onClick={() => router.push('/desserts')}>
              Back to Desserts
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div>
            ...
            {relatedDishes.length > 0 && (
              <div>
                <h3 className="text-xl font-bold mb-4">Related Desserts</h3>
                <div className="space-y-4">
                  {relatedDishes.map(related => related && (
                    <div 
                      key={related.id}
                      className="group cursor-pointer"
                      onClick={() => router.push(`/desserts/${related.id}`)}
                    >
